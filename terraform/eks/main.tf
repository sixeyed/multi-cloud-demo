...
# Creates an Amazon EKS cluster with monitoring and logging enabled
...
resource "aws_iam_role" "cloudwatch_agent" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${replace(module.eks.oidc_provider_arn, "/^.*:oidc-provider//", "")}:sub" = "system:serviceaccount:amazon-cloudwatch:cloudwatch-agent"
